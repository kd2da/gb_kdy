...
public class Calc {
	 ...
	 * 클래스 자체의 객체나, 그 안에서 포함된 멤버변수, 메서드와는
	 * 메모리 상에서 구별된다고 볼 수 있다.
	 * - 그러므로 Calc 클래스 안에 static 형태로 Calc 클래스의 객체를
	 * 정의하더라도 코드상의 논리적인 연관관계를 위해 하나의 파일에서
	 * 정의하는 것일 뿐, 실제로는 Calc 클래스 자체에서 독립된 객체가 되는 것이다.
	 * - Calc 클래스 이외의 여러 곳에서 new 연산자를 사용하여 객체 할당이 가능하다면
	 * Singleton 이라는 개념 자체가 성립되지 않으므로, 외무에서 이 객체를 직접적인 접근을
	 * 못하도록 은닉시킨다.
	 * 
	 * Calendar Class도 싱글턴 패턴으로 구성되어 있다.
	 */
	
	private static Calc current;
	
	/*
	 * current 객체를 private로 지정했기 때문에
	 ...
	 * 은닉 처리되어 외부에서의 new 연산자를 사용한 객체 할당이 금지된다.
	 */
